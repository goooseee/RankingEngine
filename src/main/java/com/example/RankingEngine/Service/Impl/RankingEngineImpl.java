...
import com.example.RankingEngine.Service.RankingEngine;
...
public class RankingEngineImpl implements RankingEngine{
	...
	public List<ScoredMovieDTO> calculateWeights(Long userId, List<Movie> candidates) {
		Map<Type, Double> userVector = preferenceService.getByUserId( userId )
				.stream().collect( Collectors.toMap( UserPreference::getType, UserPreference::getWeight ) );
		if(userVector.isEmpty()) {
			log.atInfo().log("[RankingEngine] giving user = {} candidates {} on rattings", userId, candidates.size());
			return candidates
					.stream().map(m -> new ScoredMovieDTO( m.getMovieId(), Map.of("rating",m.getRating()), m.getRating()/10.0 ))
					.sorted(Comparator.comparing( ScoredMovieDTO::total ).reversed())
					.toList();
		}
		log.atInfo().log("[RankingEngine] giving user = {} candidates {} on rattings and on his preference", userId, candidates.size());
		return candidates
	...
	private ScoredMovieDTO calculateScore(Movie movie, Map<Type,Double> userVector ) {
		...
		log.atTrace().log("[RankingFlow] Movie: {} | Cos: {} | Rating: {} | Total: {}", 
	              movie.getMovieId(), String.format("%.2f", cosinus), normalizeRating, totalScore);
		...
		return new ScoredMovieDTO( movie.getMovieId(), weightsBreakdown, totalScore );
	}
